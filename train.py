"""Training script for the chess text diffusion model.

Supports single-GPU and multi-GPU (DataParallel) training with CUDA.
Falls back to CPU if no GPU is available.

Usage:
    python train.py --config configs/default.yaml
    python train.py --config configs/default.yaml --gpus 2,3,4
"""
...
def get_device_and_model(
    ...
    """Set up device and optionally wrap model with DataParallel."""
...
def train(config: dict, gpu_ids: list[int] | None = None):
    ...
    device, diffusion = get_device_and_model(diffusion, gpu_ids)
    ...
    for epoch in range(1, config["training"]["num_epochs"] + 1):
        ...
        for batch in train_loader:
            x_0 = batch.to(device)
            padding_mask = (x_0 == tokenizer.pad_id)

            if isinstance(diffusion, nn.DataParallel):
                loss = diffusion.module.compute_loss(x_0, padding_mask)
            else:
                loss = diffusion.compute_loss(x_0, padding_mask)

            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(
                diffusion.parameters(), config["training"]["grad_clip"],
            )
            optimizer.step()
            scheduler.step()
        ...
        val_loss = evaluate(diffusion, val_loader, device, tokenizer)
        ...
        if val_loss < best_val_loss:
            ...
            save_checkpoint(
                diffusion, optimizer, scheduler, epoch, global_step,
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            if isinstance(model, nn.DataParallel):
                loss = model.module.compute_loss(x_0, padding_mask)
            else:
                loss = model.compute_loss(x_0, padding_mask)
...
def main():
    parser = argparse.ArgumentParser(description="Train chess text diffusion model")
    parser.add_argument("--config", type=str, default="configs/default.yaml")
    parser.add_argument("--gpus", type=str, default=None,
                        help="Comma-separated GPU IDs, e.g. '2,3,4'")
